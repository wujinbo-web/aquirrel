...
export default class TabTable extends Component {
  ...
  getFormValues = async (dataIndex, values) => {
    let params = {
      ...
      "orderMaterialsRecord.doutNum":Number(values.doutNum)+Number(values.number),
    };
    const data = await updateOrderState(params);
    if(data.data.state="success"){
      Toast.success("成功");
      this.state.dataSource["all"][dataIndex]["doutNum"]=Number(values.doutNum)+Number(values.number);
      this.setState({});
    }
    console.log(data);
  }
  ...
  render() {
    ...
    return (
      <div className="tab-table">
        <IceContainer>
          <h2 style={{ textAlign: "center" }}>订单{id}出库单</h2>
