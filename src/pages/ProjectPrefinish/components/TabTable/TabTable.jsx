...
export default class TabTable extends Component {
  ...
  ensureMeasure = (record, index) => {
    ...
    //修改订单状态
    axios
      .get(`${API_URL}/updateInstallState.do?orderId=${record.id}&installState=1`)
      .then((response)=>{
  ...
  getOrderData = (pageIndex) => {
    ...
    //测量订单
    axios
      .get(`${API_URL}/findOrder.do?orderState=11&pageIndex=${pageIndex}`)
      .then((response)=>{
  ...
  getStatusName = (orderStatus,installState) => {
    console.log(orderStatus, installState, "看看传入有没有错误");
    switch (orderStatus) {
      case 1:
        return "建立合同订单";
      case 12:
