...
export default class TabTable extends Component {
  ...
  ensureMeasure = (record, index) => {
    ...
    //修改订单状态
    axios
      .get(`${API_URL}/updateOrderState.do?orderId=${record.id}&orderState=4`)
      .then((response)=>{
  ...
  setLinkToRoom = (record) => {
    // 订单id：record.id
    this.props.redirectUrl(record.id,record.text);
  }
  //查找总订单接口
  getOrderData = (pageIndex) => {
    ...
    //测量订单
    axios
      .get(`${API_URL}/findOrder.do?orderState=3&pageIndex=${pageIndex}`)
      .then((response)=>{
        ...
        let fileAddress,orderState,financeState,installState,adminId,customerId,text;
        if(response.data.state == "success"){
          //设置分页
          this.setState({total: response.data.Count});
          //数据组装
          let data=response.data.data.map((item)=>{
            status=this.getStatusName(item.order.orderState,item.installState); //订单状态（中文）
            ...
            return ({id,name,signer,address,createTime,status,status2,fileAddress,orderState,financeState,installState,adminId,customerId,text});
          })
          //渲染
          this.setState({
            dataSource: {all:data},
