...
export default class EditDialog extends Component {
  ...
  isChecked = (rule, value, callback) => {
    ...
    let Bon2=(Number(value) + Number(this.props.record.jputNum)  <= this.props.record.doutNum);
    if( Bon1 && Bon2 ){
      callback();
    }else{
      console.log("失败");
      callback("入货数必须大于0小于出库数");
    }
  }
      ...
      <div style={styles.editDialog}>
        ...
        >
          <Form direction="ver" field={this.field}>
            <h2>当前可入货最大数量:{Number(record.doutNum)-Number(record.jputNum)}</h2>
