...
export default class EditDialog extends Component {
  ...
  isChecked = (rule, value, callback) => {
    ...
    let Bon2=(Number(value) + Number(this.props.record.installNum)  <= Number(this.props.record.uninstallNum));
    if( Bon1 && Bon2 ){
      callback();
    }else{
      console.log("失败");
      callback("安装数必须大于0小于待安装数");
    }
  }
      ...
      <div style={styles.editDialog}>
        ...
        >
          <Form direction="ver" field={this.field}>
            ...
            <h2>当前安装最大数量:{Number(record.uninstallNum)}</h2>

            <FormItem label="请输入安装数：" {...formItemLayout}>
              <Input
