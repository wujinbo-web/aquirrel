...
export default class TabTable extends Component {
  ...
  getFormValues = async (dataIndex, values) => {
    let params = {
       ...
       "orderMaterialsRecord.installNum":Number(values.installNum)+Number(values.number),
       "orderMaterialsRecord.uninstallNum":Number(values.uninstallNum)-Number(values.number),
    };
    const data = await updateOrderState(params);
    if(data.data.state="success"){
      Toast.success("成功");
      this.state.dataSource["all"][dataIndex]["installNum"]=Number(values.installNum)+Number(values.number);
      this.state.dataSource["all"][dataIndex]["uninstallNum"]=Number(values.uninstallNum)-Number(values.number);
      this.setState({});
    }
    console.log(data);
  }
